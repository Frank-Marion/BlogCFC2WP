<cfcomponent name="Wordpress">
	<cffunction name="init" access="remote" returnType="void" output="false" hint="Initialize the component">
 	...
 	<cffunction name="insertCategory" access="public" returntype="string" output="false" hint="I insert a new category into the WP db and return the new WPID">
		...
		<cfquery name="insertCategory" datasource="#application.Wordpress.datasource#">
			INSERT INTO #application.wordPress.pfx#categories
			(cat_name,category_nicename)
			values('#arguments.catname#','#arguments.catname#')
		</cfquery>
		<cfquery name="getNewCatID" datasource="#application.Wordpress.datasource#">
			SELECT Max(cat_ID) as newCatID FROM #application.wordPress.pfx#categories
		</cfquery>
		<cfset NewCatID = getNewCatID.newCatID>
		<cfreturn NewCatID>
	</cffunction> 
 	<cffunction name="insertPost" access="public" returntype="string" output="false" hint="I insert a new post into the WP db and return the new WPID">
 	...
 	<cffunction name="JoinAndInsertCatMappings" access="public" returntype="numeric" output="false" hint="I translate the ids from the old post/category mapping table to a new one and then insert it into the WP mapping table.">
		...
		<cfloop query="MapTableWithBoth">
			<cfquery name="insertMapping" datasource="#application.Wordpress.datasource#">
				INSERT INTO #application.wordPress.pfx#post2cat
				(post_id,category_id)
